...
from src.config.mongo_config import MONGO_URI, PIT_ATLAS_DB_NAME, PIT_ATLAS_PARSED_DB_NAME, PIT_COLLECTION_NAME, DATAHEAP_DB_NAME
...
class Sessions:
    ...
    @staticmethod
    def insert_heap_data(data, collection: str):
        """
        Inserts data into the MongoDB collection
        Args:
            List[Dict]: Dict -- "name" - pit name, "data" - dict(ed) dataframe of points assigned to pit
            collection: str -- name of mongo db collection
        """
        session = Sessions.get_db_session(db_name=DATAHEAP_DB_NAME)
